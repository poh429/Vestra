        ...
        def _bulk_add_selected(concept_name, syms, tab):
            ...
            def _worker():
                ...
                for sym in syms:
                    ...
                    # Fetch real name
                    res = lookup_symbol(sym)
                    api_label = res.get("label", sym)
                    code = sym.replace(".TW", "").replace(".TWO", "")
                    
                    if api_label == sym or api_label == code:
                        label = code
                    elif code in api_label:
                        label = api_label
                    else:
                        label = f"{api_label} {code}"
            ...
            def _finish_bulk_add(name, added):
                try:
                    ...
                    if added > 0:
                        ...
                        if self._on_save:
                            self._on_save()
                        messagebox.showinfo("成功", f"成功加入 {added} 檔「{name}」概念股！", parent=dlg)
                        dlg.destroy()
                    else:
